...
pub trait Demuxer: Send {
    ...
    fn read_headers(&mut self, buf: &mut Box<dyn Buffered>, info: &mut GlobalInfo) -> Result<SeekFrom>;
    /// Reads an event from a data structure implementing the `Buffered` trait.
    fn read_event(&mut self, buf: &mut Box<dyn Buffered>) -> Result<(SeekFrom, Event)>;
}
